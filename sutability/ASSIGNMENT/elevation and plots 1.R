...
#peatland Indonesia
library(terra)
peatland <- st_read("D:/SPATIAL DATA ANALYSIS/ASSIGNMENT2/Indonesia_peat_lands.Geojson")

Indonesia_map<- st_read("idn_admbnda_adm0_bps_20200401.shp")


Indonesia_map<- st_simplify(Indonesia_map, dTolerance = 0.01)
peatland<- st_simplify(peatland, dTolerance = 0.01)


library(ggplot2)

ggplot() +
  geom_sf(data = Indonesia_map_simplified, fill = "white", color = "black") +
  geom_sf(data = peatland_simplified, color = "green") +
  theme_minimal()

#Rasterization method

st_bbox(Indonesia_map)
ext(st_bbox(Indonesia_map))
raster_template=rast( resolution=1500,
                      ext(st_bbox(Indonesia_map)),
                      crs= st_crs(Indonesia_map)$wkt)
peatland.sf=st_as_sf(peatland)


peatland_raster=rasterize(peatland.sf, raster_template,"shape_Area")
