...
sample_size <- round(0.3 * nrow(Roads))
sample_size
Roads_sample <- Roads[sample(seq_len(nrow(Roads)), size = sample_size), ]

Roads_buffered <- st_buffer(Roads_sample, dist = 2000)
...
plot(st_geometry(Railroads_buffered))


#grid 
grid 
set.seed(123) # Setting a seed for reproducibility
sample_size_grid <- round(0.1 * nrow(grid))
grid_sample <- grid[sample(seq_len(nrow(grid)), size = sample_size_grid), ]

grid_buffered <- st_buffer(grid, dist = 2000)
...
#potential_solar$lon <- c(115,110) 
#potential_solar$lat <- c(-4,-6)

potential_solar <- data.frame(lon = 115.6, lat = -3.6)
potential_solar[nrow(potential_solar) + 1, ] <- c(112.4, -2.3)
potential_solar[nrow(potential_solar) + 1, ] <- c(117.2, -8.6)
potential_solar[nrow(potential_solar) + 1, ] <- c(120.2, -8.6)
potential_solar[nrow(potential_solar) + 1, ] <- c(119, -2.9)
potential_solar[nrow(potential_solar) + 1, ] <- c(121.1, -3.4)
potential_solar[nrow(potential_solar) + 1, ] <- c(123.5, 0.8)
potential_solar[nrow(potential_solar) + 1, ] <- c(122.4, 0.7)

potential_solar

potential_solar_sf <- st_as_sf(potential_solar, coords = c("lon", "lat"), crs = 4326)
solar_plant
avg_year_ssrd_sf
#plot(potential_solar)
tmap_mode("view")
tm_shape(avg_year_ssrd_sf) +
  tm_dots(col="ssrd", style = "quantile", size=.01, palette = "RdYlGn")+
  tm_shape(potential_solar_sf) +
  tm_dots(size=0.01)
...
tm_shape(avg_year_ssrd_sf) +
  tm_dots(col="ssrd", style = "quantile", size=.01, palette = "RdYlGn")+
  tm_shape(potential_solar_sf) +
  tm_dots(size=0.05,col="blue4")+
  tm_shape(solar_plant) +
  tm_dots(size=0.01,col="status")+
  tm_shape(Roads) +
  ...
  tm_compass(north = 0, position = c("left", "top"), size = 1.5) +
