...
solar_plant$area_m2 = c(solar_plant$capacity_mw*1000*9.2903)
# Exclude row 11
solar_plant_excluding_outlier <- solar_plant[-11, ]
mean(solar_plant_excluding_outlier$capacity_mw)
...
radiation_to_power <- function(G, A=average_area, r=0.175, p=0.6, hours=1){
...
mean(avg_spring_ssrd_df_value$ssrd)
mean(avg_summer_ssrd_df_value$ssrd)
mean(avg_autumn_ssrd_df_value$ssrd)
mean(avg_winter_ssrd_df_value$ssrd)
62591428/4
...
total_in_mWh <-sum(solar_plant$predicted_ssrd_spring,solar_plant$predicted_ssrd_summer,
    solar_plant$predicted_ssrd_autumn,solar_plant$predicted_ssrd_winter )/1e3
print ( "projected solar plant generation is ",total_in_mWh) 
total_in_mWh
